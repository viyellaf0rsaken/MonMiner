...
def load_config():
    ...
    # dashboard.py writes a generic config. Force backend-specific values here
    # so AlphaPool never inherits PearlHash's ./pearl-miner setting.
...
def load_cmd_state():
    ...
    for key, value in default_cmd_state().items():
...
def wallet_url(wallet):
    ...
    return f"https://pearl.alphapool.tech/#miner/{wallet}"
...
def recent_log_text(max_lines=500):
...
def parse_hashrate():
    ...
    # AlphaPool log format:
    # miner status attempts=... hashrate_th_s=168.88 ...
    alpha_patterns = [
        r"\bhashrate_th_s=([0-9]+(?:\.[0-9]+)?)\b",
        r"\bhashrate_ths=([0-9]+(?:\.[0-9]+)?)\b",
        r"\bhashrate\s*=\s*([0-9]+(?:\.[0-9]+)?)\s*TH/s\b",
    ]
    ...
    # Generic fallback with units.
    unit_pattern = r"([0-9]+(?:\.[0-9]+)?)\s*(TH/s|GH/s|MH/s|H/s)"
    for value, unit in re.findall(unit_pattern, text, re.IGNORECASE):
        try:
...
def parse_shares():
    ...
    submitted = sum(
        ...
        if "component=share" in line and "submitted" in line
    )

    if submitted:
        return submitted, True

    for pattern in ["accepted share", "share accepted", "accepted", "result accepted"]:
        if pattern in text:
...
def parse_rejects():
    ...
    return sum(
        1
        for line in text.splitlines()
        if "reject" in line or "stale" in line
    )


def resolve_miner_path(miner_exec):
    if os.path.isabs(miner_exec):
        return miner_exec

    if miner_exec.startswith("./"):
        return os.path.join(SCRIPT_DIR, miner_exec[2:])

    return os.path.join(SCRIPT_DIR, miner_exec)
...
def start_miner(config):
    ...
    miner_exec = config.get("miner_exec", DEFAULT_MINER_EXEC)

    if not wallet:
        append_response("No wallet address configured.")
        return False

    miner_path = resolve_miner_path(miner_exec)
    ...
    if not os.access(miner_path, os.X_OK):
        append_response(f"AlphaPool miner is not executable: {miner_exec}")
        append_response("Run: chmod +x alpha-miner")
        return False

    cmd = [
        miner_exec,
        "--pool",
        config.get("pool_host", DEFAULT_POOL_HOST),
        "--address",
        wallet,
        "--worker",
        worker,
    ...
    for raw in read_pending_commands():
        ...
        if len(parts) >= 2 and parts[0] in ("show", "hide", "toggle"):
            ...
            if action == "show":
                ...
                if target in ("all", "telemetry"):
                    new_value = not all(state[key] for key in keys)
                    for key in keys:
                        state[key] = new_value
                else:
...
def run():
    ...
    # Force backend-specific values again after loading runtime config.
